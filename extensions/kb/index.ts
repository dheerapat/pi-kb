...
export default function (pi: ExtensionAPI) {
      ...
      if (summaries.length === 0 && concepts.length === 0) {
        ...
        if (summaries.length > 0) {
          ...
          for (const name of summaries) {
            ...
            lines.push(`  - [[summaries/${name}]] (source: ${source}, added: ${added})`);
          }
          lines.push("");
        }
        ...
        if (concepts.length > 0) {
          ...
          for (const slug of concepts) {
            ...
            lines.push(`  - [[concepts/${slug}]] (sources: ${srcs})`);
          }
        }
      }
  ...
  pi.registerTool({
    name: "kb_read_index",
    label: "Read KB Index",
    description: "Read the knowledge base index.md file. Shows all documents and concepts with brief descriptions.",
    parameters: Type.Object({}),
    async execute() {
      const content = readIndex() || "(index is empty — no documents or concepts yet)";
      return {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      slug: Type.String({ description: "Concept slug (e.g. 'caching-strategy')" }),
    }),
    async execute(_toolCallId, params) {
      const info = readConcept(params.slug);
      if (!info) {
        return {
          content: [{ type: "text" as const, text: `Concept "${params.slug}" not found.` }],
          details: {},
        };
      }
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      docName: Type.String({ description: "Document name slug (e.g. 'architecture')" }),
    }),
    async execute(_toolCallId, params) {
      const text = readSummary(params.docName);
      if (!text) {
        return {
          content: [{ type: "text" as const, text: `Summary "${params.docName}" not found.` }],
          details: {},
        };
      }
      return {
        content: [{ type: "text" as const, text: `## Summary: ${params.docName}\n\n${text}` }],
        details: {},
      };
    },
  });
  ...
  pi.registerTool({
    ...
    description: "Create or overwrite a summary page for a document. Use the docName passed to you in the compile instructions.",
    parameters: Type.Object({
      docName: Type.String({ description: "Document name slug (e.g. 'architecture')" }),
      content: Type.String({ description: "Full markdown summary (200-400 words)" }),
    }),
    async execute(_toolCallId, params) {
      const reg = readRegistry();
      ...
      if (entry) {
        ...
        const hash = Object.keys(reg).find((k) => reg[k].docName === params.docName);
        if (hash) {
          reg[hash] = entry;
          writeRegistry(reg);
        }
      ...
      return {
        content: [{ type: "text" as const, text: `Summary written: summaries/${params.docName}.md` }],
        details: {},
      };
    },
  });
  ...
  pi.registerTool({
    ...
    description: "Create or update a concept page. Pass ALL sources that contributed to this concept (old + new).",
    parameters: Type.Object({
      slug: Type.String({ description: "Concept slug (lowercase, hyphens, e.g. 'caching-strategy')" }),
      content: Type.String({ description: "Full markdown concept page body" }),
      sources: Type.Array(Type.String(), { description: "List of source filenames (e.g. ['architecture.md', 'design.md'])" }),
    }),
    async execute(_toolCallId, params) {
      ...
      return {
        content: [{ type: "text" as const, text: `Concept ${action}: concepts/${params.slug}.md (sources: ${params.sources.join(", ")})` }],
        details: {},
      };
    },
  });
  ...
  pi.registerTool({
    ...
    description: "Rebuild the knowledge base index.md from a COMPLETE list of all pages. Include every existing page, not just new ones.",
    parameters: Type.Object({
      entries: Type.Array(
        Type.Object({
          type: StringEnum(["summary", "concept"] as const),
          slug: Type.String({ description: "Summary docName or concept slug" }),
          brief: Type.String({ description: "One-liner description (under 120 chars)" }),
        }),
    ...
    async execute(_toolCallId, params) {
      ...
      writeIndex(index);
      return {
        content: [{ type: "text" as const, text: `Index updated: ${docLines.length} documents, ${conceptLines.length} concepts.` }],
        details: {},
      };
    },
  });
  ...
  pi.registerTool({
    ...
    description: "Delete a concept page from the knowledge base. Use during /kb-remove when the concept had only the removed document as its source.",
    parameters: Type.Object({
    ...
    async execute(_toolCallId, params) {
      ...
      if (existed) {
        return {
          content: [{ type: "text" as const, text: `Concept deleted: concepts/${params.slug}.md` }],
          details: {},
        };
      }
      return {
        content: [{ type: "text" as const, text: `Concept "${params.slug}" not found (already deleted or never existed).` }],
        details: {},
      };
    },
  });
  ...
  pi.registerTool({
    ...
    description: "Delete a summary page from the knowledge base. Use during /kb-remove.",
    parameters: Type.Object({
      docName: Type.String({ description: "Document name slug to delete" }),
    }),
    async execute(_toolCallId, params) {
      const existed = deleteSummary(params.docName);
      if (existed) {
        return {
          content: [{ type: "text" as const, text: `Summary deleted: summaries/${params.docName}.md` }],
          details: {},
        };
      }
      return {
