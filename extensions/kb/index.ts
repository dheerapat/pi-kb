...
export default function (pi: ExtensionAPI) {
            ...
            for (const fp of filePaths) {
                ...
                if (isUrl(fp)) {
                    ...
                    if (isDocNameUsed(docName, workspace)) {
                        ...
                        ctx.ui.notify(
                            `Added${wsLabel}: ${fp} → ${finalFilename}`,
                            "info",
                        );
    ...
    pi.registerCommand("kb-list", {
        ...
        handler: async (args, ctx) => {
            ...
            if (summaries.length === 0 && concepts.length === 0) {
                lines.push(
                    `KB${wsLabel} is empty. Use /kb-add to add documents.`,
                );
            } else {
    ...
    pi.registerCommand("kb-remove", {
        ...
        handler: async (args, ctx) => {
            ...
            const wsLabel = workspace ? ` [${workspace}]` : "";
            ctx.ui.notify(
                `Removing${wsLabel}: ${entry.name} (${docName})`,
                "info",
            );
    ...
    pi.registerTool({
        ...
        async execute(_toolCallId, params) {
            const existed = listConcepts(params.workspace).includes(
                params.slug,
            );
            writeConcept(
                params.slug,
