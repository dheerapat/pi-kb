...
3. **Summary:** The LLM writes a 200–400 word summary via `kb_write_summary`. Before writing:
   - A **link resolver** scans the content and validates every `[[...]]` wiki link against the live slug registry
   - Bad links (hallucinated slugs) are stripped to plain text; unprefixed links are auto-corrected (`[[slug]]` → `[[concept/slug]]`)
   - Aspirational concept links in summaries are preserved even if the concept doesn't exist yet
4. **Concepts:** The LLM writes cross-document concept pages via `kb_write_concept`:
   - Each write tracks its slug in a **session pending set** — other concepts written later in the same session can link to it before it hits disk (solves intra-session cross-linking)
   - The resolver strips links to unknown slugs in concept pages (concepts should only reference real pages)
...
- **Body** — LLM-written prose with `[[type/slug]]` wiki links
- **Footer** — **deterministic** (code-generated, not LLM): concepts list their sources, summaries list referencing concepts (synced post-compile from ground truth)

### Link validation (on every write)

Every `kb_write_summary` and `kb_write_concept` call runs through a **link resolver** that:
- Validates `[[summary/slug]]` and `[[concept/slug]]` links against files on disk
- Auto-corrects unprefixed `[[slug]]` → `[[concept/slug]]` or `[[summary/slug]]`
- Strips brackets from hallucinated slugs (keeps text, removes dead link)
- Preserves `[[concept/...]]` links in summaries for aspirational references (the concept may be written later in the same session)
...
|---|---|---|
| Bad link from LLM | Silent dead link | Stripped to plain text |
